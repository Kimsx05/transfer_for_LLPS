...
outdir <- file.path(root,"02_qc",paste0("standardMAD_",stamp))
...
filtered<-subset(obj,cells=rownames(md)[md$QC_pass])
if(ncol(filtered)!=sum(md$QC_pass))stop("QC subset count mismatch")
cp<-file.path(root,"checkpoints",paste0("01b_qc_standardMAD_3lower_5upper_",format(Sys.time(),"%Y%m%d_%H%M%S"),".qs2"))
if(file.exists(cp))stop("Refusing checkpoint overwrite")
qs2::qs_save(filtered,cp,compress_level=4,shuffle=TRUE,nthreads=threads)
...
writeLines(c("Rule D formal QC","MAD constant = 1.4826","nCount/nFeature = lower 3 MAD, upper 5 MAD","MT = upper 3 standard MAD capped at 25%","InhouseData mt_qc_available = FALSE; fail_mt = FALSE","HB >5%",paste("QC_pass",sum(md$QC_pass))),newp(outdir,"qc_standardMAD_parameters.txt"))
message("QC_STANDARDMAD_COMPLETE cells=",ncol(filtered)," checkpoint=",cp)
