...
emb <- as.data.frame(Seurat::Embeddings(obj, "pca")[,1:2,drop=FALSE])
emb$dataset <- obj$dataset; emb$orig.ident <- obj$orig.ident
p2 <- ggplot2::ggplot(emb, ggplot2::aes(PC_1, PC_2, colour=dataset)) + ggplot2::geom_point(shape=16, size=0.05, alpha=0.6) + blank
p3 <- ggplot2::ggplot(emb, ggplot2::aes(PC_1, PC_2, colour=orig.ident)) + ggplot2::geom_point(shape=16, size=0.05, alpha=0.6) + blank
