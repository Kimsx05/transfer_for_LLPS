...
fix_shape <- function(p) {
  for (i in seq_along(p$layers)) {
    p$layers[[i]]$aes_params$shape <- "circle"
    if (!is.null(p$layers[[i]]$mapping$shape)) p$layers[[i]]$mapping$shape <- NULL
  }
  p + ggplot2::guides(shape="none", colour=ggplot2::guide_legend(override.aes=list(shape="circle", size=2, alpha=1)))
}
plots <- lapply(plots, fix_shape)
