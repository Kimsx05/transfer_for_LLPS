...
start_time <- Sys.time()
log_message("03_merge_v2", "START", "reason=Seurat_5.5.1_S3_merge")
...
for (i in seq_len(nrow(manifest))) {
  log_message("03_merge_v2", "READ_START", manifest$file_path[[i]])
  ...
  objects[[i]] <- object
  log_message("03_merge_v2", "READ_END", manifest$dataset_name[[i]], "cells=", ncol(object))
}
...
write_lines_new(checkpoint, file.path(paths$logs, "01_merged_checkpoint_path_v2.txt"))
log_message("03_merge_v2", "END", "cells=", ncol(merged), "features=", nrow(merged[["RNA"]]),
