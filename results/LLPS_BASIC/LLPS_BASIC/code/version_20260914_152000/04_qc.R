...
write_tsv_new(thresholds, file.path(paths$qc, "qc_thresholds_by_orig_ident.tsv"))
...
write_tsv_new(qc_counts, file.path(paths$qc, "qc_cell_counts.tsv"))
...
), file.path(paths$qc, "qc_gene_presence.tsv"))
...
pdf_path <- file.path(paths$qc, "qc_distributions_by_orig_ident.pdf")
png_path <- file.path(paths$qc, "qc_distributions_by_orig_ident.png")
...
qc_filtered <- Seurat::subset(merged, cells = colnames(merged)[merged$QC_pass])
...
write_lines_new(checkpoint, file.path(paths$logs, "01b_qc_checkpoint_path.txt"))
log_message("04_qc", "END", "initial=", ncol(merged), "QC_pass=", ncol(qc_filtered),
