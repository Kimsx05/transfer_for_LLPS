...
start_time <- Sys.time()
log_message("05_doubletfinder_v2", "START", "reason=correct_sweep_stats_variable")

qc_path <- latest_checkpoint("^01b_qc_filtered_.*\\.qs2$")
...
cell_output_dir <- file.path(paths$doubletfinder, "cell_classifications")
    ...
    withCallingHandlers({
      ...
      list(
        parameter = data.frame(
          orig.ident = orig_ident, n_cells_preQC = preqc_n, n_cells_current = current_n,
...
write_tsv_new(parameters, file.path(paths$doubletfinder, paste0("doubletfinder_parameters_", run_stamp, ".tsv")))
write_tsv_new(summaries, file.path(paths$doubletfinder, paste0("doubletfinder_summary_", run_stamp, ".tsv")))
...
checkpoint <- timestamped_path(paths$checkpoints, "01c_doubletfinder_singlets", "qs2")
...
log_message("05_doubletfinder_v2", "END", "QC_cells=", ncol(qc_object), "singlets=", ncol(singlets),
